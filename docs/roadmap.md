...
| 4 Distribution | In progress | Reproducible unsigned candidates, clean/tag-bound builds, post-build macOS Developer ID signing/notarization, final-asset SBOM/checksum/Sigstore/attestation generation, draft-first four-target downloaded-asset smoke, single-transition immutable publication, a verified nightly workflow, stable-only candidate/public Homebrew install/audit/test plus tap automation, repository release immutability, and the project Homebrew tap repository are implemented or configured. | Install production credentials, configure the release environment for `v*` tag deployments, and complete release and tap runs on real GitHub infrastructure. |
