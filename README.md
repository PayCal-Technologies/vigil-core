...
Install the development preview from `main`:
...
`v0.1.0` is the current public release. It is a legacy bootstrap release with
raw platform binaries, completions, a manpage, and `SHA256SUMS`; it predates the
current archive, signing, SBOM, Sigstore, attestation, and v1 gate contract.

The current release workflow is designed to publish static macOS and Linux
...
Homebrew packaging automation exists but is not advertised as an install
command until the project tap has a public, tested formula. Stable release
automation install-tests the generated formula before and after publication,
then publishes the matching formula to the project tap when
...
The commands in this section require `main` or `v0.2.0-beta.1` and later.
...
- [Safety model](docs/concepts/safety-model.md)
...
- [Acceptance evidence](docs/concepts/evidence.md)
